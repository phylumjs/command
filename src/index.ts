...
export class CommandSpec implements Iterable<ArgumentSpec> {
	...
	public add(spec: ArgumentSpec) {
		if (spec.type === 'flag' && (spec.multiple || spec.default)) {
			throw new Error(`spec.multiple and spec.default can not be used with type "flag".`);
		}
		if (this._names.has(spec.name)) {
			throw new Error(`spec.name is already used: "${spec.name}"`);
		}
		if (spec.alias && this._aliases.has(spec.alias)) {
			throw new Error(`spec.alias is already used: "${spec.alias}"`);
		}
		if (spec.default && this._default) {
			throw new Error(`spec.default can only be used by one spec.`);
		}
